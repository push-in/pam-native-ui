...
class ButtonGroupAudit(AutocompleteAudit):
    VARIATIONS = (
        "Standard",
        "Connected",
        "Full width",
    ...
    BLOCK = {"Full width", "Multiple", "Long labels", "RTL connected"}
    ...
    def __init__(self, serial: str, package: str, activity: str, output: Path) -> None:
        super().__init__(
            ...
            activity,
            output,
            component_tag="p-btn-group",
            component_label="Button Group",
        )
    ...
    def dump(self, name: str) -> ET.Element:
        ...
        remote = f"/sdcard/pam-button-group-{name}.xml"
    ...
    def group_after(self, root: ET.Element, variation: str) -> ET.Element:
        ...
        groups = [
            ...
            if node.attrib.get("content-desc") == "Button Group preview"
    ...
    def scroll_step(self, width: int, height: int, upward: bool = True) -> None:
        self.assert_foreground("button-group scroll precondition")
        center = width // 2
        start, end = (
        ...
        time.sleep(0.55)
        self.assert_foreground("button-group scroll result")
    ...
    def assert_geometry(
        ...
        expected_gap = 2.0 if variation in self.CONNECTED else 8.0
        gaps = [
    ...
    def audit_interactions(
        ...
        for index, variation in enumerate(self.VARIATIONS):
            ...
            if variation == "Standard":
                self.screenshot("03-standard")
                contrasts[variation] = self.assert_selected_color(
                    group, (22, 101, 52), "03-standard"
                )
                standard_buttons = {self.text(node): node for node in self.buttons(group)}
                self.hold_and_assert_state_layer(node_bounds(standard_buttons["Week"]))
                root = self.dump("03-standard-after-held-press")
                group = self.group_after(root, variation)
                self.assert_selected(group, {"Week"}, "Standard held press")
        ...
        root, group = self.scroll_to_group(
            "Standard", width, height, "isolation", upward=False
        )
        self.assert_selected(group, final_states["Standard"], "Standard isolation revisit")
    ...
    def audit_landscape(self) -> None:
        ...
        self.wait_for_rotation(1, "button-group landscape")
        ...
        self.wait_for_rotation(0, "button-group portrait restore")

    def audit_stress(self) -> dict[str, float | int]:
        self.launch()
        root = self.dump("10-stress-baseline")
        group = self.group_after(root, "Standard")
        ...
        self.assert_selected(
            self.group_after(warmed, "Standard"), {"Month"}, "stress warm-up"
        )
        ...
        self.assert_selected(
            self.group_after(stressed, "Standard"), {"Month"}, "rapid stress"
        )
        ...
        if total_match is None or legacy_match is None or p99_match is None:
            raise AuditFailure("Android gfxinfo did not expose button-group metrics")
        total = int(total_match.group(1))
        legacy_janky = int(legacy_match.group(1))
        legacy_percent = float(legacy_match.group(2))
        p99_ms = int(p99_match.group(1))
        if total < 12 or legacy_janky > 1 or p99_ms > 17:
            raise AuditFailure(
                "button-group stress missed its frame budget: "
    ...
    def run(self) -> dict[str, object]:
        ...
        try:
            ...
            report = {
                "schemaVersion": 2,
                "component": "p-btn-group",
                "device": self.serial,
                "package": self.package,
                "resultStatus": int(ResultStatus.PASSED),
                "checks": {
                    "allFifteenVariations": True,
                    "allFortyFourTargetsExercised": True,
                    ...
                    "noOverlappingHorizontalTargets": True,
                    "standard8DpSpacing": True,
                    "connected2DpSpacing": True,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit p-btn-group on Android.")
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("/tmp/pam-button-group-android-audit")
    )
...
def main() -> int:
    args = parse_args()
    report = ButtonGroupAudit(args.serial, args.package, args.activity, args.output).run()
    print(json.dumps(report["checks"], indent=2))
    print(f"PASS p-btn-group; evidence: {args.output}")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL p-btn-group: {exception}")
        raise SystemExit(1)
