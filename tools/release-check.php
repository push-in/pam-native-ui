...
$assert(
    ($composer['require']['pushinbr/pam-native'] ?? null) === '^1.0.27',
    'The package must require the pushinbr/pam-native:^1.0.27 contract.',
);
...
$assert(
    ($plugin['pamNative']['minimum'] ?? null) === '1.0.27'
...
$assert(
    ($exampleComposer['require']['pushinbr/pam-native-ui'] ?? null) === $version
        && ($exampleComposer['require']['pushinbr/pam-native'] ?? null) === '1.0.27',
    'The kitchen sink must exercise immutable PAM Native UI and core 1.0.27 releases.',
);
...
$assert(
    is_string($releaseWorkflow)
        && str_contains($releaseWorkflow, "tags:\n")
        && str_contains($releaseWorkflow, '- "v*"'),
    'Native release must certify version tags.',
);
$assert(
    is_string($composerWorkflow)
        && str_contains($composerWorkflow, "release:\n")
        && str_contains($composerWorkflow, '- published'),
